...
export class ShareholdingComponent {
  ...
  ngOnInit(): void {
   ...
   fetchDocsList() {
    ...
    this.apiService.docsList('User','User','Active',0, 'shareholding_patterns').subscribe({
      next:(response: any) => {
  ...
  return 'assets/img/'+map[ext] || 'icon-file.png';   // default icon
   ...
   uploadPdfs(){
    this.http.get(`${environment.PdfUpload}`).subscribe((response:any) => {
      this.filterCategoty = response.filter(category=>category.uploads_category==='Shareholding Patterns')
