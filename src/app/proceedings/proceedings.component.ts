...
export class ProceedingsComponent {
  ...
  fetchDocsList() {
    ...
    this.apiService.docsList('User', 'User', 'Active', 0, 'postal_ballot_proceedings').subscribe({
      next: (response: any) => {
...
getFileIconFromName(filename: string): string {
   ...
   uploadPdfs(){
    this.http.get(`${environment.PdfUpload}`).subscribe((response:any) => {
      this.filterCategoty = response.filter(category=>category.uploads_category==='Proceedings')
