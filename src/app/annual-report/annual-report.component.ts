...
export class AnnualReportComponent {
  ...
  ngOnInit(): void {
    ...
    this.apiService.docsList('User','User','Active',0, 'annual_report').subscribe({
      next:(response: any) => {
  ...
  return 'assets/img/'+map[ext] || 'icon-file.png';   // default icon
   ...
   uploadPdfs(){
    this.http.get(`${environment.PdfUpload}`).subscribe((response:any) => {
      this.filterCategoty = response.filter(category=>category.uploads_category==='Annual Reports')
