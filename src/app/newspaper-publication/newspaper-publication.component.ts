...
@Component({
  ...
  styleUrls: ['./newspaper-publication.component.css']
})
...
fetchDocsList() {
    ...
    this.apiService.docsList('User', 'User', 'Active', 0, 'newspaper_publication').subscribe({
      next: (response: any) => {
...
getFileIconFromName(filename: string): string {
   ...
   uploadPdfs(){
    this.http.get(`${environment.PdfUpload}`).subscribe((response:any) => {
      this.filterCategoty = response.filter(category=>category.uploads_category==='Newspaper Publication')
