...
@Component({
  selector: 'app-corporate-announcements',
  templateUrl: './corporate-announcements.component.html',
  styleUrls: ['./corporate-announcements.component.css']
})
export class CorporateAnnouncementsComponent {
          docs: any;
  mediaUrlDocs!: string;
...
fetchDocsList() {
    ...
    this.apiService.docsList('User', 'User', 'Active', 0, 'corporate_announcements').subscribe({
      next: (response: any) => {
    ...
    this.http.get(`${environment.PdfUpload}`).subscribe((response:any) => {
      this.filterCategoty = response.filter(
        (category) =>
          ...
          'Corporate Announcements under Reg 30(8)'
      );
