...
@Component({
  ...
  styleUrls: ['./related-party-transaction.component.css']
})
...
fetchDocsList() {
    ...
    this.apiService.docsList('User', 'User', 'Active', 0, 'related_party_transaction').subscribe({
      next: (response: any) => {
...
getFileIconFromName(filename: string): string {
  ...
  uploadPdfs() {
    this.http.get(`${environment.PdfUpload}`).subscribe((response: any) => {
      this.filterCategoty = response.filter(category => category.uploads_category === 'Related Party Transaction')
