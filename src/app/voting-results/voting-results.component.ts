...
export class VotingResultsComponent {
 ...
 fetchDocsList() {
    ...
    this.apiService.docsList('User', 'User', 'Active', 0, 'postal_ballot_voting_results').subscribe({
      next: (response: any) => {
...
getFileIconFromName(filename: string): string {
   ...
   uploadPdfs(){
    this.http.get(`${environment.PdfUpload}`).subscribe((response:any) => {
      this.filterCategoty = response.filter(category=>category.uploads_category==='Voting results as per regulation 44/ Scrutinizer Report')
